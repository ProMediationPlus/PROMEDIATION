  ...
  const renderFormComponent = (formId: string) => {
    switch (formId) {
      ...
      default:
        ...
        if (savedForm) {
          // Here you would render a generic form viewer or the specific component if known
          // For now, let's just show a placeholder
          return <div className="p-4">Displaying saved form: {savedForm.formTitle}</div>;
        }
                  ...
                  {savedForms.map((form, index) => (
                    <Card key={index}>
                      ...
                      <CardFooter className={isMobile ? "p-3" : ""}>
                        <Button variant="outline" size={isMobile ? "sm" : "default"} className="w-full" onClick={() => console.log("Reuse form:", form)}>
                          <Plus className={`${iconSizeClass} mr-1.5`} />
                          Use Form
                        </Button>
                      </CardFooter>
