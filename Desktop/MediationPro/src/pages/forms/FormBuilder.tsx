...
interface TemplateBuilderProps {
    onFormSave?: () => void; // Optional callback for when a form is saved
    initialData?: StatementOfMeansData | null; // Prop to pass initial data for editing
}

export function TemplateBuilder({ onFormSave, initialData }: TemplateBuilderProps) {
    ...
    useEffect(() => {
        if (initialData) {
            ...
            // Reset to default new form values if no initialData (e.g. creating a brand new form)
            form.reset({
                id: uuidv4(),
                formTitle: "New Form",
                formDescription: "",
    ...
    function onSubmit(data: StatementOfMeansData) {
        ...
        try {
            // Ensure the data conforms to the SavedForm interface
            const dataToSave: SavedForm = {
                id: data.id, // id is guaranteed by schema default
                formTitle: data.formTitle, // formTitle is guaranteed by schema .min(1) and .default()
                formDescription: data.formDescription,
                statementDate: data.statementDate,
                sections: data.sections, // sections are guaranteed by schema .default([])
            };
            
            console.log("Data being passed to putItem:", JSON.stringify(dataToSave, null, 2));
            
            putItem("forms", dataToSave); 
            toast.success(initialData ? "Form updated successfully!" : "Form saved successfully!");
