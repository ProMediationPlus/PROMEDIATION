...
// Placeholder for Meeting interface - Define properly as needed
...
// This should align with what you're saving from FormBuilder.tsx
export interface SavedForm {
  id: string; // Unique identifier for the form
  formTitle: string;
  formDescription?: string;
  statementDate?: string;
  sections: any[]; // Define more strictly if possible, based on your sectionSchema
}
...
export interface MediatorMateDBSchema extends DBSchema {
  ...
  meetings: {
    key: string;
    value: Meeting; // Assuming Meeting type is defined elsewhere or you can use a generic object
    indexes: { 'by-caseId': string; 'by-date': string }; // Index by caseId and date
  };
  forms: { // Added forms store
    key: string;
    value: SavedForm; // Using the new SavedForm interface
    indexes: { 'by-formTitle': string }; // Example index
  };
