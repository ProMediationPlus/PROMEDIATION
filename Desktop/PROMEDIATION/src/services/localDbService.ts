...
const DATABASE_VERSION = 5; // Incremented version due to adding 'forms' store
  ...
  if (!dbPromise) {
    dbPromise = openDB<MediatorMateDBSchema>(DATABASE_NAME, DATABASE_VERSION, {
      upgrade(db, oldVersion, newVersion, transaction) {
        ...
        // Add the new forms store if it doesn't exist (added in v5)
        if (!db.objectStoreNames.contains('forms')) {
          const formsStore = db.createObjectStore('forms', { keyPath: 'id' });
          formsStore.createIndex('by-formTitle', 'formTitle'); // Index by formTitle
          console.log("Created 'forms' object store.");
        }
...
type StoreNameUnion = "matters" | "notes" | "contacts" | "documents" | "tasks" | "caseFiles" | "timeline" | "meetings" | "forms"; // Added "forms"
