            ...
            >
              <CheckSquare 
                className={`${isMobile ? "h-4 w-4" : "h-5 w-5"} ${
                  task.status === "Done" ? "text-green-500" : "text-gray-400" // Changed to "Done"
                }`} 
              />
            </button>
            <div className="ml-1"> {/* Adjusted ml-3 to ml-1 */}
              <p className={`${isMobile ? "text-xs" : "text-sm"} font-medium ${
                task.status === "Done" ? "line-through text-muted-foreground" : "" // Changed to "Done"
              }`}>
                {task.title}
              </p>
              {/* Add Link to case summary page below task title */}
              <Link 
                to={`/case-files/${task.caseId}/summary`}
          ...
          <div className="flex items-center space-x-0">
            ...
            <EditTaskDialog 
              ...
              onSave={(updatedTask) => handleSaveTask({
                ...
                status: (updatedTask.status || "Pending") as "Todo" | "In Progress" | "Done" | "Blocked", // Cast and provide default
